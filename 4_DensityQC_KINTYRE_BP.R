...
setwd("M:/PHD_2ndYR/Recombination_ROH")
unaff <- read.table("BP_ROH_output/KINTYRE_ROH_updatedMB_Sanger2022.hom.summary", header = T, stringsAsFactors = F)
###############################################################################
...
cor.test(pos_win$UNAFF_n, pos_win$UNAFF_mean)
pos_win$UNAFF_n <- factor(pos_win$UNAFF_n) ##only for boxplot 
ggplot(pos_win, aes(UNAFF_n, UNAFF_mean)) +  geom_boxplot()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(x="Number of SNPs in 1500Kb window", y="Mean number of ROH called in 1500Kb window", title = "Kintyre BP QC")+
  geom_vline(xintercept=23.5, colour="red", size=1)
## num of ROH found was correlated with number of snps in a window 
...
for(i in 1:nrow(snps_filt)){
  snps_filt$pall[i] <- snps_filt$UNAFF[i]/157
}

mean(snps_filt$pall) ##mean pall cm= 0.029 (11/01/2021) lower than before when had less snps
sem<-sd(snps_filt$pall)/sqrt(length(snps_filt$pall)) # standard error pall
sem
quantile(snps_filt$pall, c(.01, .99)) ## 
length(which(snps_filt$pall > 0.0955414)) ##324 hotspot snps
length(which(snps_filt$pall <= 0)) ## 3727 


write.table(snps_filt,
            file = "Data_files/Kintyre_BP_UpdatedMbSANGER_filtSNPS.txt",
