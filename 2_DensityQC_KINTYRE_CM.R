...
setwd("H:/PHD_2ndYR/Recombination_ROH")
unaff <- read.table("CM_ROH_output/KINTYRE_ROH_trueCMpos.hom.summary", header = T, stringsAsFactors = F)
###############################################################################
...
pos_win$UNAFF_n <- factor(pos_win$UNAFF_n)
ggplot(pos_win, aes(UNAFF_n, UNAFF_mean)) +  geom_boxplot()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(x="Number of SNPs in 1500Kb window", y="Mean number of ROH called in 1500Kb window", title = "Kintyre CM QC")+
  geom_vline(xintercept=24, colour="red", size=1)
...
mean(snps_filt$pall) ##mean pall cm= 0.032 (11/01/2021) lower than before when had less snps
sem<-sd(snps_filt$pall)/sqrt(length(snps_filt$pall)) # standard error pall
sem
quantile(snps_filt$pall, c(.01, .99)) ## 
length(which(snps_filt$pall > 0.08280255)) ##92 hotspot snps
length(which(snps_filt$pall <= 0)) ## 3280 


#write.table(snps_filt,
 #           file = "KINTYRE_Pall_CM_filt_moreSNPs.txt",
