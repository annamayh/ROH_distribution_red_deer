...
unaff <- read.table("CM_ROH_output/ROH_output_trueCM.hom.summary", header = T, stringsAsFactors = F)
###############################################################################
...
pos_win <- winScan(x = unaff_rem, 
                   ...
                   values = c("UNAFF"), 
                   win_size = 1500,
                   win_step = 100, #step every 500 kb
                   funs = c("mean"))
...
ggplot(pos_win, aes(UNAFF_n, UNAFF_mean)) +  geom_boxplot()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  labs(x="Number of SNPs in 1500Kb window", y="Mean number of ROH called in 1500Kb window", title = "Rum CM QC")+
...
mean(snps_filt$pall) ##mean pall cm= 0.060857 (11/01/2021) lower than before when had less snps
sem<-sd(snps_filt$pall)/sqrt(length(snps_filt$pall)) # standard error pall
sem
quantile(snps_filt$pall, c(.01, .99)) ## 
length(which(snps_filt$pall > 0.1486795)) ##257 hotspot snps
length(which(snps_filt$pall <= 0)) ## 549 


#write.table(snps_filt,
 #           file = "Pall_CM_filt_moreSNPs.txt",
